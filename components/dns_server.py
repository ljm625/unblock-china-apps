...
class Resolver(ProxyResolver):
    def __init__(self, upstream, zone_data):
        super(Resolver,self).__init__(upstream, 53, 5)
        self.records = self.get_zone_data(zone_data)
    ...
    def get_zone_data(self,datas):
        zones=[]
        for data in datas:
            record = Record(data['rname'],data['rtype'],[data['args']])
            zones.append(record)
        return zones
